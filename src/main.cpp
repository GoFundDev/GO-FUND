...
#if defined(NDEBUG)
#error "GO-FUND cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are disabled"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        CAmount nValueIn = 0;
        if(tx.IsZerocoinSpend()){
            return state.Invalid(error("AcceptToMemoryPool : zerocoin is disabled."));
        } else {
            LOCK(pool.cs);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from GFC
...
{
    // GFC: recalculate Accumulator Checkpoints that failed to database properly
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // GFC: We use certain sporks during IBD, so check to see if they are
