...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GFC);
    ...
    if (!settings.contains("nAnonymizeGfcAmount"))
        settings.setValue("nAnonymizeGfcAmount", 1000);

    nAnonymizeGfcAmount = settings.value("nAnonymizeGfcAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGfcAmount"))
        SoftSetArg("-anonymizeGfcamount", settings.value("nAnonymizeGfcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gofund.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGfcAmount:
            return QVariant(nAnonymizeGfcAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGfcAmount:
            nAnonymizeGfcAmount = value.toInt();
            settings.setValue("nAnonymizeGfcAmount", nAnonymizeGfcAmount);
            emit anonymizeGfcAmountChanged(nAnonymizeGfcAmount);
            break;
        case CoinControlFeatures:
