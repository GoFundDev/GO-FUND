...
// Copyright (c) 2015-2018 The GFCX developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::GFC)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sGFCPercentage, QString& szGFCPercentage)
{
    int nPrecision = 2;
    ...
    szGFCPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sGFCPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount gfcAvailableBalance = balance - immatureBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = gfcAvailableBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // GFC labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, gfcAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zGFC.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", true);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in gofund.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in gofund.conf");
    }
    ...
    bool showGFCAvailable = settingShowAllBalances || gfcAvailableBalance != nTotalBalance;
    bool showWatchOnlyGFCAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showGFCPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyGFCPending = watchUnconfBalance != 0;
    bool showGFCLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyGFCLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showGFCAvailable || showWatchOnlyGFCAvailable);
    ui->labelBalanceText->setVisible(showGFCAvailable || showWatchOnlyGFCAvailable);
    ui->labelWatchAvailable->setVisible(showGFCAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showGFCPending || showWatchOnlyGFCPending);
    ui->labelPendingText->setVisible(showGFCPending || showWatchOnlyGFCPending);
    ui->labelWatchPending->setVisible(showGFCPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showGFCLocked || showWatchOnlyGFCLocked);
    ui->labelLockedBalanceText->setVisible(showGFCLocked || showWatchOnlyGFCLocked);
    ui->labelWatchLocked->setVisible(showGFCLocked && showWatchOnly);
